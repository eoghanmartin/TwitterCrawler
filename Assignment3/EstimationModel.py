...
class EstModel():
	...
	def Ztj(self, j):
		# This puts everything together to determine the latent variables - probability of claims being true
		# According to equation 11
		prob_of_truth_when_true = self.Atj(j) * self.d
		both_true_when_true_and_true_when_false = self.Atj(j) * self.d + self.Btj(j) * (1 - self.d)
		total_probability = prob_of_truth_when_true / float(both_true_when_true_and_true_when_false)
